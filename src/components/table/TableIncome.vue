<template>
    <div>
        ...
        <table class="w-full text-sm text-left">
            ...
            <div v-if="selectedItem" aria-hidden="true" class="fixed top-0 right-0 bottom-0 left-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
                <div class="relative p-4 w-full max-w-2xl max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="p-4 md:p-5 space-y-4">
                            ...
                            <p class="text-base leading-relaxed text-gray-500">
                                {{ getSubcategoryName(selectedItem.subcategory_id) }}
                            </p>
                            <p class="text-base leading-relaxed text-gray-500">
                                {{ formatRussianDate(selectedItem.created_at) }}
                            </p>
                        </div>
                        <!-- Modal footer -->
                        <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                            ...
                            <button type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10">
                                Decline
                            </button>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        openModal(id) {
            this.selectedItem = this.incomeData.find(item => item.id === id);
