<template>
    ...
    <div v-for="(item, index) in data" :key="index" class="bg-white border-b hover:bg-gray-50">
        <div :id="`accordion-open-${item.id}`" data-accordion="open">
            ...
            <!-- Accordion body -->
            <div v-if="item.data" :id="`accordion-open-body-${item.id}`" class="hidden" :aria-labelledby="`accordion-open-heading-${item.id}`">
                <table class="p-5 border border-t-0 border-gray-200 dark:border-gray-700 w-full">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 w-full">
                        ...
                        <th class="px-3 py-2">Вес/объем</th>
                        <th class="px-3 py-2">Цена за 1 ед / полный вес</th>
                        <th class="px-3 py-2">Субкатегория</th>
                    </thead>
                    <tbody>
